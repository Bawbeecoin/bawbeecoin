...
uint256 hashGenesisBlock("0x2902c0af53e22af2d4c0a09879858f20a2e8f756c08de0c5384be9dbcb7c2d3e");
...
const string strMessageMagic = "Bawbeecoin Signed Message:\n";
...
{
    // Bawbeecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bawbeecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bawbeecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bawbeecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bawbeecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bawbeecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bawbeecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2902c0af53e22af2d4c0a09879858f20a2e8f756c08de0c5384be9dbcb7c2d3e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 12/Feb/2014 Scottish independence, UK parties will block money union";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1393759057;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087326101; //2084524493;
        ...
        assert(block.hashMerkleRoot == uint256("0xd7693e1e05cb65780ce58f89ce84ed233a6fb8d7cf77320a132989cc06869deb"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BawbeecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BawbeecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BawbeecoinMiner(CWallet *pwallet)
{
    printf("BawbeecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bawbeecoin-miner");
    ...
    try { loop {
        ...
        printf("Running BawbeecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BawbeecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BawbeecoinMiner, pwallet));
}
