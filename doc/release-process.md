...
**Perform Mac build:**
  ...
  OSX binaries are created on an OS X 10.8 (Mountain Lion) VM by iAtkos.
  Bawbeecoin/BAWBag 0.8.x is built with MacPorts and QT5 (Not 4)

	qmake RELEASE=1 USE_UPNP=1 USE_QRCODE=1
	make
	export QTDIR=/opt/local/share/qt5  # needed to find translations/qt_*.qm files
	T=$(contrib/qt_translations.py $QTDIR/translations src/qt/locale)
	python2.7 share/qt/clean_mac_info_plist.py
	python2.7 contrib/macdeploy/macdeployqtplus BAWBag.app -add-qt-tr $T -dmg -fancy contrib/macdeploy/fancy.plist

 Build output expected: BAWBag.dmg
